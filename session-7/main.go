...
import (
	...
	"github.com/rpturbina/learn-go-fga/belajar-gorm/database"
...
func main() {
	...
	// createUser("rizki@gmail.com")
	// getUserById(1)
	// updateUserById(1, "pratama@gmail.com")
...
func updateUserById(id uint, email string) {
	...
	if err != nil {
		fmt.Println("Errpr updating user data:", err)
		return
	}
...
func getUsersWithProducts() {
	...
	users := models.User{}
	err := db.Preload("Products").Where("users.id = 1").Find(&users).Error
